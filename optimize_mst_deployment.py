#!/usr/bin/env python3
"""
Optimize MST deployment by consolidating MSTs within 250m of each other.
Drop cables are inexpensive, so it's better to have one MST with longer drops
than multiple MSTs close together.
"""
...
def optimize_mst_deployment(terminals, min_distance=250.0):
    """
    Consolidate MSTs that are within min_distance (250m) of each other.
    Keep the MST with more ONTs or better position, merge others into it.
    """
    print()
    print("=" * 80)
    print("OPTIMIZING MST DEPLOYMENT")
    print("=" * 80)
    print()
    print(f"Consolidating MSTs within {min_distance}m of each other...")
    print()
    
    # Get all MSTs
    msts = [t for t in terminals if t.get("type") == "MST"]
    print(f"Total MSTs: {len(msts)}")
    
    # Build MST position map
    mst_positions = {}
    for mst in msts:
    ...
    # Find MSTs within min_distance of each other
    msts_to_remove = set()
    consolidations = []
    
    for i, mst1 in enumerate(msts):
        mst1_id = mst1.get("terminal_id", "")
        if mst1_id in msts_to_remove:
            continue
        
        mst1_pos = mst_positions.get(mst1_id)
        if not mst1_pos:
            continue
        
        mst1_onts = len(mst1.get("connected_onts", []))
        ...
        for j, mst2 in enumerate(msts):
            ...
            mst2_id = mst2.get("terminal_id", "")
            if mst2_id in msts_to_remove:
                continue
            
            mst2_pos = mst_positions.get(mst2_id)
            if not mst2_pos:
                continue
            
            dist = euclidean_distance(mst1_pos[0], mst1_pos[1], mst2_pos[0], mst2_pos[1])
            
            if dist <= min_distance:
                mst2_onts = len(mst2.get("connected_onts", []))
                nearby_msts.append((mst2_id, mst2, dist, mst2_onts))
        
        if nearby_msts:
            # Decide which MST to keep
            # Prefer MST with more ONTs, or if equal, keep the first one
            all_msts_in_group = [(mst1_id, mst1, 0.0, mst1_onts)] + nearby_msts
            all_msts_in_group.sort(key=lambda x: (x[3], -x[2]), reverse=True)  # Sort by ONT count (desc), then distance (asc)
            
            kept_mst_id, kept_mst, _, _ = all_msts_in_group[0]
            removed_msts = all_msts_in_group[1:]
            
            # Merge ONTs from removed MSTs into kept MST
            kept_mst_obj = next((m for m in msts if m.get("terminal_id") == kept_mst_id), None)
            if kept_mst_obj:
                total_onts_before = len(kept_mst_obj.get("connected_onts", []))
                
                for removed_id, removed_mst_obj, dist_to_kept, _ in removed_msts:
                    removed_onts = removed_mst_obj.get("connected_onts", [])
                    kept_mst_obj["connected_onts"].extend(removed_onts)
                    msts_to_remove.add(removed_id)
                ...
                total_onts_after = len(kept_mst_obj.get("connected_onts", []))
                
                print(f"  ✓ Consolidated: Kept {kept_mst_id} ({total_onts_before} → {total_onts_after} ONTs)")
                for removed_id, _, dist, _ in removed_msts:
                    print(f"    - Removed {removed_id} ({dist:.1f}m away)")
    
    # Remove consolidated MSTs
    optimized_terminals = [t for t in terminals if t.get("terminal_id") not in msts_to_remove]
    
    print()
    print(f"Consolidated {len(consolidations)} MST pairs")
    print(f"Removed {len(msts_to_remove)} redundant MSTs")
    print(f"Final MST count: {len([t for t in optimized_terminals if t.get('type') == 'MST'])}")
    print()
    
    return optimized_terminals, consolidations


def main():
    print("=" * 80)
    print("MST DEPLOYMENT OPTIMIZATION")
    print("=" * 80)
    print()
    ...
    # Optimize MST deployment
    optimized_terminals, consolidations = optimize_mst_deployment(terminals, min_distance=250.0)
    
    # Save optimized terminals
    with open("test_output/optimized_terminals.json", "w") as f:
        json.dump(optimized_terminals, f, indent=2)
    
    print("✓ Saved optimized terminals")
    print()
    ...
    with open("test_output/optimized_visualization.geojson", "w") as f:
        json.dump(vis_geojson, f, indent=2)
    
    print(f"✓ Saved visualization: test_output/optimized_visualization.geojson")
    ...
    print(f"  - {len(optimized_terminals)} terminals")
