#!/usr/bin/env python3
"""
Create final visualization with all rules applied.
"""
...
def main():
    ...
    # Stub cables (MST to FOSC) - MUST run along fiber cables
    from utils.cable_routing import route_stub_cable_along_fiber
    
    # Build cables list for routing (use ALL cables, not just extracted, for routing)
        ...
        if terminal_type == "MST" and connected_fosc_id and terminal_pos:
            ...
            if fosc:
                ...
                if fosc_pos:
                    ...
                    # Check if there's a preferred cable for routing (e.g., specific cable path)
                    preferred_cable_id = None
                    # For specific terminals, use preferred cable routing
                    # T0004288 and T0004601 should route along 48FOC/F1000398/T1001731
                    if terminal_id in ["T0004288", "T0004601"] and connected_fosc_id == "F0000962":
                        preferred_cable_id = "48FOC/F1000398/T1001731"
                    # Also check if terminal's connected cable matches the preferred cable
                    elif connected_cable_id == "48FOC/F1000398/T1001731" and connected_fosc_id == "F0000962":
                        preferred_cable_id = "48FOC/F1000398/T1001731"
                    
                    path, length = route_stub_cable_along_fiber(
                        term_pos_utm,
                        fosc_pos_utm,
                        cables_list,
                    ...
                    stub_cable = {
                        ...
                        "properties": {
                            ...
                            "length_m": length,
                            "routed_along_cable": len(path) > 2,  # More than 2 points means routed
                            "stroke": "#00FF00",
                            "stroke-width": 3,
                            "stroke-opacity": 0.8
                        }
    ...
    # Drop cables (Terminal/MST to ONT) - Direct connections, NOT routed along fiber cables
    drop_cables = []
    for terminal in terminals:
        terminal_id = terminal.get("terminal_id", "")
        terminal_pos = terminal.get("position")
    ...
    with open("test_output/final_rules_optimized.geojson", "w") as f:
        json.dump(vis_geojson, f, indent=2)
    
    print(f"✓ Saved visualization: test_output/final_rules_optimized.geojson")
    ...
    print(f"  - {len(terminals)} terminals ({len([t for t in terminals if t.get('type') == 'MST'])} MSTs)")
